...
MainClass::MainClass(QObject *parent) : QObject(parent) {
  ...
  cclient->frecebeAutorizacao(cabaConfig->instance()->fgetUsername(),
                              cabaConfig->instance()->fgetAuthCode());
  cclient->fbaixaListas();

  cconfiguracoesUsuarioDiretorios->instance()->frecebeConfigs(
      cabaConfig->instance()->fgetDirectory().toStringList());
...
void MainClass::fconnections() {
  ...
  connect(cclient, &Client::sdownloadCompleted, this,
          &MainClass::ftryClientConnection);
  ...
  connect(vtimerChecaAssistindo, &QTimer::timeout, this,
          QOverload<>::of(&MainClass::fchecaAnimeAssistido));

  connect(vtimerAutoRefresh, &QTimer::timeout, cclient,
          QOverload<>::of(&Client::fbaixaListas));
  connect(vtimerCountdown, &QTimer::timeout, this,
          QOverload<>::of(&MainClass::fupdateTimer));
  connect(vtimerUpdateClient, &QTimer::timeout, this,
          QOverload<>::of(&MainClass::fclientUpdate));
}
...
void MainClass::fdownloadCoverImages() {
  ...
  if (cdownloader->isBusy() && !vlistaFilaTipo.isEmpty()) {
    ...
    cdownloader->setListAndType(vlistaFilaLista.takeFirst(),
                                vlistaFilaTipo.takeFirst());
    //    cdownloader->setListAndType(vlistaFilaLista.first(),
    //    vlistaFilaTipo.first());
...
void MainClass::fconnectSuccess() {
  ...
  cleitorListaAnimes->instance()->fleJson();
  vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
      vordemLista, vlistaAtual, vtipoAtual);
  ...
  if (!tthreadDiretorios.isRunning()) {
    ...
    cconfiguracoesUsuarioDiretorios->instance()->moveToThread(
        &tthreadDiretorios);
...
void MainClass::fconnectFail() {
  cclient->frecebeAutorizacao(cabaConfig->instance()->fgetUsername(),
                              cabaConfig->instance()->fgetAuthCode());
  // If the connection fails, we will try to read the anime list and connect
...
void MainClass::fclientUpdate() {
  ...
  foreach (QStringList key, vlistaAcoes.keys()) {
    nome = cleitorListaAnimes->instance()->fbuscaAnimePorIDERetornaTitulo(
        key.at(1));
    if (vrateLimitRequests == 90)
      ...
      if (cclient->fmudaNota(key.at(1).toInt(), vlistaAcoes[key].toInt())) {
        ...
        qDebug()
            << "Couldn't reach the client server. Trying again in 10 seconds.";
    } else if (key.at(0) == "progresso") {
      if (cclient->fmudaProgresso(key.at(1).toInt(),
    ...
    //            }
    //            else
    //                qDebug() << "Couldn't reach the client server. Trying
    //                again in 10 seconds.";
    //        }
...
void MainClass::fsaveConfig() { cabaConfig->instance()->fsaveSettings(); }
...
void MainClass::fsetTorrentList() { cabaTorrent->fgetTorrentList(); }
...
void MainClass::fsearchAnimeFromTorrent(QVariant posicaoTabela) {
  QVariant infoAnime =
      cabaTorrent->fgetSingleTorrentInfo(posicaoTabela.toInt());
  if (!infoAnime.toInt())
...
void MainClass::fbotaoBusca(QVariant search) {
  ...
  if (vjanelaAtual == janela::MAIN) {
    if (!searchText.isEmpty()) {
      ...
      vlistaSelecionada = cleitorListaAnimes->instance()->fbuscaLista(
          searchText.simplified(), vtipoAtual);
      if (!vlistaSelecionada.isEmpty()) {
        vlistaAtual = enumlistaToQString(lista::SEARCH);
        vindexAnimeSelecionado = 0;
        vpagina = 1;
        //            ui->NumPagina->setText("Busca  - " +
        //            QString::number(vlistaSelecionada.size()) +
        //                                   " animes in the list - Page
        //                                   "+QString::number(vpagina)+"/"+
...
void MainClass::fbotaoDownloadTorrents() { cabaTorrent->fdownloadAnimes(); }

void MainClass::finfoAnimeSelecionado(QVariant posicaoAnimeNaGrid) {
  emit sdirImagensMedias(QVariant(
      cconfiguracoesDiretoriosPadrao->instance()->vdiretorioImagensMedio));
  emit sdirImagensGrandes(QVariant(
      cconfiguracoesDiretoriosPadrao->instance()->vdiretorioImagensGrandes));
  ...
  if (vlistaSelecionada.size() > vindexAnimeSelecionado) {
    emit snomeAnimeSelecionado(
        QVariant(vlistaSelecionada[vindexAnimeSelecionado]->vnome));
    emit snomeAlternativoAnimeSelecionado(
        QVariant(vlistaSelecionada[vindexAnimeSelecionado]->vnomeIngles));
    emit ssinopseAnimeSelecionado(
        QVariant(vlistaSelecionada[vindexAnimeSelecionado]->vsinopse));
    emit sstatusAnimeSelecionado(
        QVariant(vlistaSelecionada[vindexAnimeSelecionado]->vstatus));
    emit sseasonAnimeSelecionado(
        QVariant(vlistaSelecionada[vindexAnimeSelecionado]->vseason));
    emit smediaGloballAnimeSelecionado(QVariant(QString::number(
        vlistaSelecionada[vindexAnimeSelecionado]->vnotaMediaSite.toFloat() /
            10,
        'f', 2)));
    emit smediaPessoalAnimeSelecionado(
        QVariant(vlistaSelecionada[vindexAnimeSelecionado]->vnotaMediaPessoal));
    emit sreleaseAnimeSelecionado(
        QVariant(vlistaSelecionada[vindexAnimeSelecionado]->vdataEpisodio));
    ...
    else if (vlistaSelecionada[vindexAnimeSelecionado]
                 ->vnumProximoEpisodioLancado.toInt() == 0)
      emit sepisodiosLancadosAnimeSelecionado(QVariant("All Episodes"));
    else {
      if (vlistaSelecionada[vindexAnimeSelecionado]
                  ->vnumProximoEpisodioLancado.toInt() -
              1 ==
          1)
        emit sepisodiosLancadosAnimeSelecionado(
            QVariant(QString::number(vlistaSelecionada[vindexAnimeSelecionado]
                                         ->vnumProximoEpisodioLancado.toInt() -
                                     1) +
                     " Episode"));
      else if (vlistaSelecionada[vindexAnimeSelecionado]
    ...
    emit sepisodiosAssistidosAnimeSelecionado(QVariant(
        vlistaSelecionada[vindexAnimeSelecionado]->vnumEpisodiosAssistidos));
    emit sepisodiosTotaisAnimeSelecionado(QVariant(
        vlistaSelecionada[vindexAnimeSelecionado]->vnumEpisodiosTotais));
    emit stipoAnimeSelecionado(
        QVariant(vlistaSelecionada[vindexAnimeSelecionado]->vformato));
    QString episodePath =
        carquivos->fprocuraEpisodio(vlistaSelecionada[vindexAnimeSelecionado]);
    if (!episodePath.isEmpty())
      emit sproximoEpisodioAnimeSelecionado(true);
    else
      emit sproximoEpisodioAnimeSelecionado(false);
    QPixmap pix;
    if (pix.load(cconfiguracoesDiretoriosPadrao->instance()
                         ->vdiretorioImagensGrandes +
                     vlistaSelecionada[vindexAnimeSelecionado]->vid + ".png",
                 "png"))
      emit simagemAnimeSelecionado(QVariant(
          cconfiguracoesDiretoriosPadrao->instance()->vdiretorioImagensGrandes +
          vlistaSelecionada[vindexAnimeSelecionado]->vid + ".png"));
    else if (pix.load(cconfiguracoesDiretoriosPadrao->instance()
                              ->vdiretorioImagensGrandes +
      ...
      emit simagemAnimeSelecionado(QVariant(
          cconfiguracoesDiretoriosPadrao->instance()->vdiretorioImagensMedio +
          vlistaSelecionada[vindexAnimeSelecionado]->vid));
    emit sidAnimeSelecionado(
        QVariant(vlistaSelecionada[vindexAnimeSelecionado]->vid));
  }
  fmostraListaAnimes();
}
...
QVariant MainClass::fretornaNomeAnimePosicao(QVariant posicao) {
  if (vlistaSelecionada.size() > (12 * (vpagina - 1)) + posicao.toInt())
    return QVariant(
        vlistaSelecionada[(12 * (vpagina - 1)) + posicao.toInt()]->vnome);
  return QVariant("");
}
QVariant MainClass::fretornaEpisodiosAnimePosicao(QVariant posicao) {
  if (vlistaSelecionada.size() > (12 * (vpagina - 1)) + posicao.toInt())
    return QVariant("Progresso: " +
                    vlistaSelecionada[(12 * (vpagina - 1)) + posicao.toInt()]
                        ->vnumEpisodiosAssistidos +
                    "/" +
                    vlistaSelecionada[(12 * (vpagina - 1)) + posicao.toInt()]
                        ->vnumEpisodiosTotais);
  return QVariant("");
...
QVariant MainClass::fretornaListaAnimePosicao(QVariant posicao) {
  if (vlistaSelecionada.size() > (12 * (vpagina - 1)) + posicao.toInt())
    return QVariant(
        vlistaSelecionada[(12 * (vpagina - 1)) + posicao.toInt()]->vlista);
  return QVariant("");
}

QVariant MainClass::fretornaEpisodioAnimeEncontrado(QVariant posicao) {
  if (vlistaSelecionada.size() > (12 * (vpagina - 1)) + posicao.toInt()) {
    QString episodePath = carquivos->fprocuraEpisodio(
        vlistaSelecionada[(12 * (vpagina - 1)) + posicao.toInt()]);
    if (!episodePath.isEmpty())
      return QVariant("1");
    else if (vlistaSelecionada[(12 * (vpagina - 1)) + posicao.toInt()]
                 ->vstatus.compare("Not Aired Yet", Qt::CaseInsensitive) == 0)
      return QVariant("-1");
  }
  return QVariant("0");
}
...
/// TODO
QVariant MainClass::fretornaNomeUsuario() { return QVariant("gutocbs"); }
...
void MainClass::fordemLista(QVariant ordem) {
  // First we need to see if the order is the same, and if it is, we will change
  // the first letter To invert the order
  ...
  // If it's not, let's change to the new order, with a C to make it from low to
  // high
  else
...
void MainClass::fabreSite(QVariant data) {
  ...
  if (ok) {
    QDesktopServices::openUrl(
        QUrl(vlistaSelecionada[vindexAnimeSelecionado]->vsiteAnilist));
  }
}
...
/// Se a função de todas as listas forem iguais, posso fazer uma função e passar
/// o lista::Current como referência
void MainClass::fselecionaListaCurrent() {
  ...
  case leitorlistaanimes::type::SEASON:
    if (vlistaAtual.compare(enumlistaToQString(lista::WINTER) +
                                QString::number(vanoBuscaAnimes),
                            Qt::CaseInsensitive) != 0) {
      vlistaAtual =
          enumlistaToQString(lista::WINTER) + QString::number(vanoBuscaAnimes);
      // Change the old list to the new
      vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
          vordemLista, vlistaAtual, vtipoAtual);
      // Check if the list is empty for some reason
  ...
  default:
    ...
    if (vlistaAtual.compare(enumlistaToQString(lista::CURRENT),
                            Qt::CaseInsensitive) != 0) {
      vlistaAtual = enumlistaToQString(lista::CURRENT);
      // Change the old list to the new
      vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
...
void MainClass::fselecionaListaCompleted() {
  ...
  case leitorlistaanimes::type::SEASON:
    if (vlistaAtual.compare(enumlistaToQString(lista::SPRING) +
                            ...
                            Qt::CaseInsensitive) != 0) {
      vlistaAtual =
          enumlistaToQString(lista::SPRING) + QString::number(vanoBuscaAnimes);
      // Change the old list to the new
      vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
          vordemLista, vlistaAtual, vtipoAtual);
      // Check if the list is empty for some reason
  ...
  default:
    // Check if it is already the right list
    if (vlistaAtual.compare(enumlistaToQString(lista::COMPLETED),
                            Qt::CaseInsensitive) != 0) {
...
void MainClass::fselecionaListaPaused() {
  ...
  case leitorlistaanimes::type::SEASON:
    if (vlistaAtual.compare(enumlistaToQString(lista::SUMMER) +
                            ...
                            Qt::CaseInsensitive) != 0) {
      vlistaAtual =
          enumlistaToQString(lista::SUMMER) + QString::number(vanoBuscaAnimes);
      // Change the old list to the new
      vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
          vordemLista, vlistaAtual, vtipoAtual);
      // Check if the list is empty for some reason
  ...
  default:
    // Check if it is already the right list
    if (vlistaAtual.compare(enumlistaToQString(lista::PAUSED),
                            Qt::CaseInsensitive) != 0) {
...
void MainClass::fselecionaListaDropped() {
  ...
  case leitorlistaanimes::type::SEASON:
    if (vlistaAtual.compare(enumlistaToQString(lista::FALL) +
                            ...
                            Qt::CaseInsensitive) != 0) {
      vlistaAtual =
          enumlistaToQString(lista::FALL) + QString::number(vanoBuscaAnimes);
      // Change the old list to the new
      vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
          vordemLista, vlistaAtual, vtipoAtual);
      // Check if the list is empty for some reason
  ...
  default:
    // Check if it is already the right list
    if (vlistaAtual.compare(enumlistaToQString(lista::DROPPED),
                            Qt::CaseInsensitive) != 0) {
...
void MainClass::fselecionaListaPlanning() {
  // Check if it is already the right list
  if (vlistaAtual.compare(enumlistaToQString(lista::PLANNING),
                          Qt::CaseInsensitive) != 0) {
...
void MainClass::fabreProximoEpisodio() {
  // Search for the next episode in the computer. If it exists, a string with
  // it's path will be returned. Otherwise, will be an empty string.
  QString episodePath =
      carquivos->fprocuraEpisodio(vlistaSelecionada[vindexAnimeSelecionado]);
  if (!episodePath.isEmpty()) {
...
void MainClass::fabrePastaAnime() {
  QByteArray lcaminhoPasta =
      cconfiguracoesUsuarioDiretorios->instance()
          ->fretornaDiretorioEspecifico(
              vlistaSelecionada[vindexAnimeSelecionado]->vid.toInt())
          .toUtf8();
...
void MainClass::fabreStream() {
  QDesktopServices::openUrl(
      QUrl(vlistaSelecionada[vindexAnimeSelecionado]->vstreamCrunchyroll));
}

void MainClass::frefresh() { cclient->fbaixaListas(); }
...
void MainClass::fchecaAnimeAssistido() {
  ...
  QString nomejanela;
  // Pego o nome de todos os players selecionados nas configurações para
  // reconhecimento
  ...
  foreach (const auto &janela, vlistaJanelas) {
    if (!QString::fromStdString(janela.GetTitle()).isEmpty()) {
      ...
      foreach (QString player, lplayers) {
        if (player.isEmpty())
          break;
        // Algumas janelas não especificam o player no nome, então temos que
        // incluí-los de acordo com o padrão Do titulo da janela e a extensão do
        // arquivo
        ...
        // Caso a janela do player tenha sido encontrada, é hora de descobrir
        // qual anime está sendo visto.
        if (nomejanela.contains(player, Qt::CaseInsensitive)) {
          ...
          QString vidAnime =
              cleitorListaAnimes->instance()->fprocuraNomeRetornaID(nomeAnime);
          if (!vidAnime.isEmpty()) {
            emit sanimeReconhecidoID(QVariant(vidAnime), QVariant(nomeAnime),
                                     QVariant(episodio));
            // Vai checar a cada 10 segundos, então tem que assistir dois
            // minutos de anime pra contar. Assim, atualiza os curtas ao mesmo
...
void MainClass::fAumentaProgressoID(const QString &ridAnime,
                                    const QString &episodioAnime) {
  // A função vai rodar tudo em uma lista temporária e dar refresh na lista
  // atual, pra mudar o progresso Para isso, é preciso ter a posição e a lista do
  // anime salvas
  QString llistaAtual = vlistaSelecionada[vindexAnimeSelecionado]->vlista;
  int lposicao =
      cleitorListaAnimes->instance()->fbuscaAnimePorIDERetornaPosicao(ridAnime);
  QString llistaAnimeAssistindo =
      cleitorListaAnimes->instance()->fbuscaAnimePorIDERetornaLista(ridAnime);
  ...
  if (llistaAnimeAssistindo.compare("Watching", Qt::CaseInsensitive) == 0) {
    ...
    if (ok) {
      listaTemp = cleitorListaAnimes->instance()->fleListaAno(
          llistaAnimeAssistindo.toInt());
    }
  }
  ...
  if (lposicao != -1 && listaTemp[lposicao]->vnumEpisodiosAssistidos.toInt() <
    ...
    for (int i = 0;
      ...
      if (listaTemp[lposicao]->vnumEpisodiosAssistidos.toInt() <
        ...
        cleitorListaAnimes->instance()->fmudaProgresso(
        ...
        if (listaTemp[lposicao]->vnumEpisodiosAssistidos.toInt() ==
          ...
          cleitorListaAnimes->instance()->fmudaLista(
              listaTemp[lposicao]->vid, listaTemp[lposicao]->vlista,
              leitorlistaanimes::type::ANIME);
        }
  ...
  if (llistaAnimeAssistindo.compare("On Hold", Qt::CaseInsensitive) == 0) {
    vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
        vordemLista, "PAUSED", leitorlistaanimes::type::ANIME);
  } else if (llistaAnimeAssistindo.compare("Dropped", Qt::CaseInsensitive) ==
             0) {
    vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
        vordemLista, "DROPPED", leitorlistaanimes::type::ANIME);
  } else if (llistaAnimeAssistindo.compare("Plan to Watch",
                                           Qt::CaseInsensitive) == 0) {
    vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
        vordemLista, "PLANNING", leitorlistaanimes::type::ANIME);
  } else if (llistaAnimeAssistindo.compare("Completed", Qt::CaseInsensitive) ==
             0) {
    vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
        vordemLista, "COMPLETED", leitorlistaanimes::type::ANIME);
  } else {
    vlistaSelecionada = cleitorListaAnimes->instance()->sortLista(
        vordemLista, "CURRENT", leitorlistaanimes::type::ANIME);
  }
...
void MainClass::fmudaProgresso(QVariant progresso) {
  if (progresso.toString().compare("diminui") == 0) {
    if (vlistaSelecionada[vindexAnimeSelecionado]
            ->vnumEpisodiosAssistidos.toInt() > 0) {
      vlistaSelecionada[vindexAnimeSelecionado]->vnumEpisodiosAssistidos =
          QString::number(vlistaSelecionada[vindexAnimeSelecionado]
      ...
      finfoAnimeSelecionado(vposicaoGridAnimeSelecionado);
      QString lacao =
          "progresso:" + vlistaSelecionada[vindexAnimeSelecionado]->vid;
      QStringList lstringListAcao = lacao.split(':');
      vlistaAcoes.insert(
          lstringListAcao,
          QString::number(vlistaSelecionada[vindexAnimeSelecionado]
    ...
    if (vlistaSelecionada[vindexAnimeSelecionado]->vnumEpisodiosTotais != "?")
      lepisodiosTotais = vlistaSelecionada[vindexAnimeSelecionado]
                             ->vnumEpisodiosTotais.toInt();
    else
      lepisodiosTotais = INT_MAX;
    if (vlistaSelecionada[vindexAnimeSelecionado]
            ->vnumEpisodiosAssistidos.toInt() < lepisodiosTotais) {
      vlistaSelecionada[vindexAnimeSelecionado]->vnumEpisodiosAssistidos =
          QString::number(vlistaSelecionada[vindexAnimeSelecionado]
                              ->vnumEpisodiosAssistidos.toInt() +
                          1);
      ...
      if (vlistaSelecionada[vindexAnimeSelecionado]
              ->vnumEpisodiosAssistidos.toInt() ==
          vlistaSelecionada[vindexAnimeSelecionado]
              ->vnumEpisodiosTotais.toInt()) {
        fmudaListaAnime("COMPLETED");
      } else if ((vlistaSelecionada[vindexAnimeSelecionado]->vlista.compare(
                      "Completed", Qt::CaseInsensitive) != 0)) {
        vlistaSelecionada[vindexAnimeSelecionado]->vlista = "Watchig";
        cclient->fmudaLista(
            vlistaSelecionada[vindexAnimeSelecionado]->vid.toInt(), "CURRENT");
        cleitorListaAnimes->instance()->fmudaLista(
            vlistaSelecionada[vindexAnimeSelecionado]->vid,
...
void MainClass::fmudaNota(QVariant nota) {
  if (nota.toString().compare("diminui") == 0) {
    if (vlistaSelecionada[vindexAnimeSelecionado]->vnotaMediaPessoal.toInt() >
        0) {
      vlistaSelecionada[vindexAnimeSelecionado]->vnotaMediaPessoal =
          QString::number(vlistaSelecionada[vindexAnimeSelecionado]
                              ->vnotaMediaPessoal.toInt() -
                          1);
      emit smediaPessoalAnimeSelecionado(QVariant(
          vlistaSelecionada[vindexAnimeSelecionado]->vnotaMediaPessoal));
      cleitorListaAnimes->instance()->fmudaNota(
          vlistaSelecionada[vindexAnimeSelecionado]->vid,
          vlistaSelecionada[vindexAnimeSelecionado]->vnotaMediaPessoal);
      finfoAnimeSelecionado(vposicaoGridAnimeSelecionado);
...
void MainClass::fmostraListaAnimes() {
  ...
  if (vlistaSelecionada.size() > (12 * (vpagina - 1)) + 10)
    emit sidAnime11(
        QVariant(vlistaSelecionada[(12 * (vpagina - 1)) + 10]->vid));
  else
    emit sidAnime11(QVariant("null"));
  if (vlistaSelecionada.size() > (12 * (vpagina - 1)) + 11)
    emit sidAnime12(
        QVariant(vlistaSelecionada[(12 * (vpagina - 1)) + 11]->vid));
  else
...
void MainClass::fmudaListaAnime(QVariant rnewListVariant) {
  ...
  case leitorlistaanimes::type::ANIME:
    if (rnewList.compare("CURRENT") == 0) {
      ...
      cclient->fmudaLista(
          vlistaSelecionada[vindexAnimeSelecionado]->vid.toInt(), "COMPLETED");
      cleitorListaAnimes->instance()->fmudaLista(
          vlistaSelecionada[vindexAnimeSelecionado]->vid,
          vlistaSelecionada[vindexAnimeSelecionado]->vlista, vtipoAtual);
      ...
      cclient->fmudaLista(
          vlistaSelecionada[vindexAnimeSelecionado]->vid.toInt(), "PAUSED");
      cleitorListaAnimes->instance()->fmudaLista(
          vlistaSelecionada[vindexAnimeSelecionado]->vid,
          vlistaSelecionada[vindexAnimeSelecionado]->vlista, vtipoAtual);
      ...
      cclient->fmudaLista(
          vlistaSelecionada[vindexAnimeSelecionado]->vid.toInt(), "DROPPED");
      cleitorListaAnimes->instance()->fmudaLista(
          vlistaSelecionada[vindexAnimeSelecionado]->vid,
          vlistaSelecionada[vindexAnimeSelecionado]->vlista, vtipoAtual);
      ...
      cclient->fmudaLista(
          vlistaSelecionada[vindexAnimeSelecionado]->vid.toInt(), "PLANNING");
      cleitorListaAnimes->instance()->fmudaLista(
          vlistaSelecionada[vindexAnimeSelecionado]->vid,
          vlistaSelecionada[vindexAnimeSelecionado]->vlista, vtipoAtual);
    }
  ...
  case leitorlistaanimes::type::SEASON:
    if (rnewList.compare("CURRENT") == 0) {
      ...
      cleitorListaAnimes->instance()->fmudaLista(
          vlistaSelecionada[vindexAnimeSelecionado]->vid,
          vlistaSelecionada[vindexAnimeSelecionado]->vlista,
          leitorlistaanimes::type::ANIME);
    } else if (rnewList.compare("COMPLETED") == 0) {
      vlistaSelecionada[vindexAnimeSelecionado]->vlista = "Completed";
      cclient->fmudaLista(
