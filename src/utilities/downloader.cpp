...
void Downloader::fsetWorker() {
  ...
  connect(dw, &DownloaderWorker::finishedSpecificXML, this,
          &Downloader::finishedSpecificXML);
  connect(dw, &DownloaderWorker::finishedAnimeTorrent, this,
          &Downloader::finishedAnimeTorrent);
...
void Downloader::setAvatar(QString urlAvatar) { dw->fsetURLAvatar(urlAvatar); }
...
void Downloader::finished() { checkwork(); }
...
void Downloader::finishedBig() { checkworkBig(); }
...
void Downloader::finishedAvatar() { checkworkAvatar(); }
...
void Downloader::checkworkAnimeTorrent() {
  ...
  foreach (DownloaderWorker *dw, m_workers) {
    if (!dw->isBusy() && !torrentLink.isEmpty() && !torrentName.isEmpty()) {
      dw->fdownloadAnimeTorrent(torrentLink.takeFirst(),
                                torrentName.takeFirst(), m_work.first());
      m_work.takeFirst();
      if (m_work.isEmpty())
        return;
    }
