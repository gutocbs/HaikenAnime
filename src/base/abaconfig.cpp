...
abaConfig *abaConfig::createInstance() { return new abaConfig(); }
...
QVariant abaConfig::fgetDirectory() { return QVariant(vanimesDirectory); }
...
void abaConfig::fsaveSettings() {
  ...
  foreach (QString dir, vanimesDirectory) { directories.append(dir); }
  map[QString(vmetaEnumConfig.valueToKey(configs::DIRECTORY))] =
      QVariant(directories);
...
void abaConfig::floadSettings() {
  ...
  QVariantList vlist = qvariant_cast<QVariantList>(
      document[QString(vmetaEnumConfig.valueToKey(configs::DIRECTORY))]);
  foreach (QVariant key, vlist) { vanimesDirectory.append(key.toString()); }
...
void abaConfig::fsetService(QVariant client) {
  ...
  vsettings.setValue("service",
                     QString(vmetaEnumClient.valueToKey(vclientEscolhido)));
}
...
void abaConfig::fsetDetection() {
  ...
  if (!vdirConfigMap.contains("detection") ||
      (vdirConfigMap.contains("detection") &&
       vdirConfigMap["detection"] != det)) {
...
void abaConfig::fsetPlayers() {
  ...
  players.removeAll("");
  if (!vdirConfigMap.contains("players") ||
      (vdirConfigMap.contains("players") &&
       vdirConfigMap["players"] != players)) {
    vdirConfigMap.insert("players", players);
  }
}

bool abaConfig::fgetHighQualityImages() { return vhighQualityImages; }

QStringList abaConfig::fgetDetection() { return vdirConfigMap["detection"]; }

QStringList abaConfig::fgetPlayers() { return vdirConfigMap["players"]; }

void abaConfig::fsetGeneralFeed() {
  QStringList generalFeed;
  generalFeed.append(vsettings.value("confTorrent/generalFeed").toString());
  if (!vtorConfigMap.contains("generalFeed") ||
      (vtorConfigMap.contains("generalFeed") &&
...
void abaConfig::fsetSpecificFeed() {
  ...
  if (!vtorConfigMap.contains("specificFeed") ||
      (vtorConfigMap.contains("specificFeed") &&
...
bool abaConfig::fsetDefaultTorrentSoftware() {
  ...
  // Checa se os softwares existem, e caso existam, os insere como software de
  // escolha
  for (int i = 0; i < nomeTorrents.size(); i++) {
    if (QFile::exists(settings.value("ProgramFilesDir").toString() +
                      QDir::separator() + nomeTorrents.at(i) +
                      QDir::separator() + exeTorrents.at(i))) {
      vtorConfigMap.insert(
          "preferredTorrentPath",
          QStringList(settings.value("ProgramFilesDir").toString() +
                      QDir::separator() + nomeTorrents.at(i) +
                      QDir::separator() + exeTorrents.at(i)));
      return true;
    } else if (QFile::exists(
                   settings.value("ProgramFilesDir (x86)").toString() +
                   QDir::separator() + nomeTorrents.at(i) + QDir::separator() +
                   exeTorrents.at(i))) {
      vtorConfigMap.insert(
          "preferredTorrentPath",
          QStringList(settings.value("ProgramFilesDir (x86)").toString() +
                      QDir::separator() + nomeTorrents.at(i) +
                      QDir::separator() + exeTorrents.at(i)));
      return true;
    }
...
void abaConfig::fsetTorrentOptions() {
  ...
  if (!vtorConfigMap.contains("autodownloadTimer") ||
      (vtorConfigMap.contains("autodownloadTimer") &&
       vtorConfigMap["autodownloadTimer"] !=
           vsettings.value("confTorrent/autodownloadTimer").toStringList())) {
    vtorConfigMap.insert(
        "autodownloadTimer",
        vsettings.value("confTorrent/autodownloadTimer").toStringList());
  }
  QStringList config =
      vsettings.value("confTorrent/downloadLists").toStringList();
  config.removeDuplicates();
  config.removeAll("");
  if (!vtorConfigMap.contains("downloadLists") ||
      (vtorConfigMap.contains("downloadLists") &&
       vtorConfigMap["downloadLists"] != config)) {
    vtorConfigMap.insert("downloadLists", config);
  }
  if (!vtorConfigMap.contains("preferredSub") ||
      (vtorConfigMap.contains("preferredSub") &&
       vtorConfigMap["preferredSub"] !=
           vsettings.value("confTorrent/preferredSub").toStringList())) {
    vtorConfigMap.insert(
        "preferredSub",
        vsettings.value("confTorrent/preferredSub").toStringList());
  }
  if (!vtorConfigMap.contains("preferredQuality") ||
      (vtorConfigMap.contains("preferredQuality") &&
       vtorConfigMap["preferredQuality"] !=
           vsettings.value("confTorrent/preferredQuality").toStringList())) {
    vtorConfigMap.insert(
        "preferredQuality",
        vsettings.value("confTorrent/preferredQuality").toStringList());
  }
  if (!vtorConfigMap.contains("preferredTorrent") ||
      (vtorConfigMap.contains("preferredTorrent") &&
       vtorConfigMap["preferredTorrent"] !=
           vsettings.value("confTorrent/preferredTorrent").toStringList())) {
    vtorConfigMap.insert(
        "preferredTorrent",
        vsettings.value("confTorrent/preferredTorrent").toStringList());
  }
  if (!vtorConfigMap.contains("preferredTorrentPath") ||
      (vtorConfigMap.contains("preferredTorrentPath") &&
       vtorConfigMap["preferredTorrentPath"] !=
           vsettings.value("confTorrent/preferredTorrentPath")
               .toStringList())) {
    ...
    if (!vsettings.value("confTorrent/saveFolder").toStringList().isEmpty()) {
      ...
      if (saveFolder.exists())
        vtorConfigMap.insert(
            "preferredTorrentPath",
            vsettings.value("confTorrent/preferredTorrentPath").toStringList());
      else {
  ...
  if (!vtorConfigMap.contains("saveFolder") ||
      (vtorConfigMap.contains("saveFolder") &&
       vtorConfigMap["saveFolder"] !=
           vsettings.value("confTorrent/saveFolder").toStringList())) {
