...
void abaTorrent::fRefreshControl() {
  // Primeiramente, devemos checar se a lista está vazia. Caso não esteja,
  // devemos esvaziá-la
...
void abaTorrent::fgetTorrentList() {
  ...
  connect(downloaderTorrent, &Downloader::sfinishedXML, this,
          &abaTorrent::freadTorrentList);
}
...
void abaTorrent::freadTorrentList() {
  ...
  while (!stream.atEnd()) {
    ...
    case QXmlStreamReader::StartElement:
      if (stream.name() == "title") {
        ...
        if (lresolucao.compare("720p") == 0 ||
            lresolucao.compare("480p") == 0 || lresolucao.compare("360p") == 0)
          lresolucao.insert(0, "0");

        // Tento achar o id do anime de forma rápida, pra pegar informações com
        // mais facilidade
        if (vHashDeNomeEId.contains(lnomeAnime))
          ...
          lid = cleitorListaAnimes->instance()->fprocuraIdNasListasRapido(
              lnomeAnime);
        if (!vHashDeNomeEId.contains(lnomeAnime))
          vHashDeNomeEId.insert(lnomeAnime, lid);

        lepisodiosAssistidos =
            cleitorListaAnimes->instance()->fbuscaAnimePorIDERetornaEpisodio(
                lid);
        lista =
            cleitorListaAnimes->instance()->fbuscaAnimePorIDERetornaLista(lid);
        lposicaoAnimeNaLista =
            cleitorListaAnimes->instance()->fbuscaAnimePorIDERetornaPosicao(
                lid);
      }
      if (stream.name() == "link") {
      ...
      if (stream.name() == "description") {
        ...
        // 0 -> não baixar
        // 1 -> Não é a qualidade ou sub certo, mas pode ser baixado na falta de
        // melhores opções 2 -> Qualidade ou sub certo, pode ser baixado na falta
        // de melhores opções 3 -> Arquivo perfeito, qualidade e sub certo, será
        // baixado.
        lprioridade = fcheckPriority(ltorrentAux);
        ltorrentAux->vprioridade = lprioridade;

        // Checa se o anime existe na lista de downloads, separando cada
        // episódio como uma entrada diferente Caso já contenha uma entrada,
        // checa se a prioridade da entrada na lista é menor do que a atual A
        // ideia é baixar sempre os que tem maior prioridade
        if (vdownloadList.contains(lnomeAnime + lepisodioAnime)) {
          // As entradas são formadas por uma combinação de prioridade e posição
          // do torrent na lista.
          if (vdownloadList.value(lnomeAnime + lepisodioAnime).at(0).toInt() <
              lprioridade)
            // A entrada atual, caso tenha menos prioridade do que o torrent
            // checado, será sobrescrita
            vdownloadList.insert(lnomeAnime + lepisodioAnime,
                                 (QString::number(lprioridade) + ":" +
                                  QString::number(vlistaTorrents.size()))
                                     .split(":"));
        }
        // Caso o anime não exista na lista de downloads, será inserido na lista
        // independente de sua prioridade Desde que seja maior que 0
  ...
  foreach (QString key, vdownloadList.keys()) {
    if (vlistaTorrents.size() > vdownloadList[key].at(1).toInt()) {
      ...
      // Isso acontece com o propósito de facilitar a ordenação ao montar a
      // tabela
...
void abaTorrent::fchangeTorrentState(int posicaTabela, bool checkState) {
  if (vinfoTorrent.isEmpty() ||
      vinfoTorrent.at(posicaTabela).toStringList().isEmpty())
    return;
  QList<QVariant> listaOrdenada = fgetTorrentInfo(vordemLista);
  QStringList tempList = vinfoTorrent.at(posicaTabela).toStringList();
  //-3 por que a posição na lista está no terceiro elemento de frente pra trás
  //na QStringList
...
void abaTorrent::fdownloadAnimes() {
  ...
  if (vlistaAnimesBaixados.isEmpty()) {
    for (int i = 0; i < vlistaTorrents.size(); i++) {
      if (vlistaTorrents[i]->vbaixar) {
        ...
        downloaderTorrent->workAnimeTorrent(idDownloader++,
                                            vlistaTorrents[i]->vlinkTorrent,
                                            vlistaTorrents[i]->vnomeTorrent);
        vlistaTorrents[i]->vbaixar = false;
        connect(downloaderTorrent, &Downloader::sfinishedAnimeTorrent, this,
                &abaTorrent::fdownloadAnimes);
        return;
      }
    }
  } else {
  ...
  // Caso ele rode o vetor todo e não tenha mais nenhum download, ele sai e
  // começa a carregar os torrents no programa
...
void abaTorrent::fdownloadTorrents() {
  if (cabaConfig->instance()->fgetSaveFolder().isEmpty()) {
    //        ui->labelMensagem->setText("Error: Trying to download without
  ...
  for (int i = 0; i < tamanhoLista; i++) {
    ...
    //        argumentos.append(QDir::currentPath() +
    //        "/Configurações/Temp/Torrents/" +
    //        torrent[vlistaDownload[0]]->vnomeTorrent +
    //                ".torrent");
    //    }
    if (cabaConfig->instance()->fgetPreferredTorrentPath().contains(
            "qbittorrent")) {
      lprocesso->setProgram(cabaConfig->instance()->fgetPreferredTorrentPath());
      argumentos.append("--add-paused=false");
      argumentos.append("--skip-dialog=true");
      argumentos.append(
          "--save-path=" + cabaConfig->instance()->fgetSaveFolder() + "/" +
          vlistaTorrents[vlistaAnimesBaixados[0]]->vnomeAnime);
      argumentos.append(QDir::currentPath() + "/Configurações/Temp/Torrents/" +
                        vlistaTorrents[vlistaAnimesBaixados[0]]->vnomeTorrent +
                        ".torrent");
    }
    lprocesso->setArguments(argumentos);
    if (lprocesso->startDetached()) {
      QString id = cleitorListaAnimes->instance()->fprocuraNomeRetornaID(
          vlistaTorrents[vlistaAnimesBaixados[0]]->vnomeAnime);
      cconfPastasAnimes->instance()->fselecionaPastaEspecificaAnime(
          id, cabaConfig->instance()->fgetSaveFolder() + "/" +
                  vlistaTorrents[vlistaAnimesBaixados[0]]->vnomeAnime);
...
int abaTorrent::fcheckPriority(torrentinfo *rtorrent) {
  ...
  // Checa se o anime está em uma lista que deve ser baixada. Se não estiver, a
  // prioridade é 0
  QStringList listas = cabaConfig->instance()->fgetDownloadLists();
  if ((rtorrent->vlista.compare("Watching", Qt::CaseInsensitive) == 0 &&
       listas.contains("WATCHING")) ||
      (rtorrent->vlista.compare("Plan to Watch", Qt::CaseInsensitive) == 0 &&
       listas.contains("PLANNING")) ||
      (rtorrent->vlista.compare("On Hold", Qt::CaseInsensitive) == 0 &&
       listas.contains("PAUSED")) ||
      (rtorrent->vlista.compare("Dropped", Qt::CaseInsensitive) == 0 &&
       listas.contains("DROPPED")))
    prioridade++;
  else
    return 0;

  // Checa se o episódio já foi assistido, e caso já tenha a prioridade é 0
  if (cleitorListaAnimes->instance()->fbuscaAnimePorIDERetornaEpisodio(
          vHashDeNomeEId[rtorrent->vnomeAnime]) >= rtorrent->vepisodioAnime)
    return 0;
  ...
  if (!carquivos
           ->fprocuraEpisodioEspecifico(
               cleitorListaAnimes->instance()->fretornaAnimePorID(
                   vHashDeNomeEId[rtorrent->vnomeAnime]),
               rtorrent->vepisodioAnime.toInt())
           .isEmpty()) {
    return 0;
  }

  // Checa se o episódio tem as legendas do sub escolhido e, se tiver, aumenta a
  // prioridade
  if (rtorrent->vfansub.compare(cabaConfig->instance()->fgetPreferredSub(),
                                Qt::CaseInsensitive) == 0)
    prioridade++;
  // Checa se o episódio tem a qualidade escolhida e, se tiver, aumenta a
  // prioridade
  if (rtorrent->vresolucao.compare(
          cabaConfig->instance()->fgetPreferredQuality(),
          Qt::CaseInsensitive) == 0)
    prioridade++;
...
QList<QVariant> abaTorrent::fgetTorrentInfo(QString ordem) {
  ...
  if (ordem.startsWith("c")) {
    if (ordem.contains("nomeAnime")) {
      std::sort(vinfoTorrent.begin(), vinfoTorrent.end(),
                [](QVariant &a, QVariant &b) -> bool {
...
// return
// QString("true§"+vnomeAnime+"§"+vfansub+"§"+vresolucao+"§"+vepisodioAnime+"§"+vnomeTorrent+"§"
